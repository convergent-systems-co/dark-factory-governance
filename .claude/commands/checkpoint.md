...
3. **Write a checkpoint file** to `.checkpoints/{timestamp}-{branch}.json` where `{timestamp}` is `YYYYMMDD-HHMMSS` and `{branch}` is the current branch name (with slashes replaced by dashes). The file must contain:
   ```json
   {
...
5. **Offer two options**:
   - **Option A (recommended if context is high):** Run `/clear` to reset context, then resume with: `Resume from checkpoint: .checkpoints/{checkpoint-file}`
