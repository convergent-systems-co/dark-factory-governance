...
- **Spawn parallel worker agents** — use the `Task` tool with `isolation: "worktree"` and `run_in_background: true` to dispatch up to N Coder/IaC Engineer agents concurrently (N = `governance.parallel_coders` from `project.yaml`, default 5). Each agent receives its full persona instructions (Coder or IaC Engineer as appropriate), the plan, acceptance criteria, and branch name. All independent issues are dispatched in a single message with multiple Task tool calls for maximum concurrency.
