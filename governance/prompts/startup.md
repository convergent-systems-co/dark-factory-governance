...
**Activation:** Set `governance.use_project_manager: true` in `project.yaml`. When this flag is absent or `false`, the standard pipeline (Phases 0-5 above) operates unchanged.

When active, the Project Manager (`governance/personas/agentic/project-manager.md`) replaces the DevOps Engineer as the session entry point and introduces multiplexed Code Managers for higher throughput. The standard phases are restructured as follows:
...
flowchart TD
    ...
    DE_BG -->|RESULT grouped batches| PM2[PM Phase 2: Dispatch Code Managers]
    DE_BG -.->|WATCH new issues| PM2

    PM2 -->|"Task(worktree, background)"| CM1[Code Manager 1]
    PM2 -->|"Task(worktree, background)"| CM2[Code Manager 2]
    PM2 -->|"Task(worktree, background)"| CMM[Code Manager M]
...
| 2b | Code Managers (parallel) | Each CM plans its batch, dispatches its Coders (nested parallelism: PM -> CM -> Coder) |
| 3 | Project Manager | Collects Code Manager RESULTs, coordinates cross-batch dependencies, processes WATCH messages |
...
Identical to standard Phase 0. The Project Manager executes checkpoint scanning, validation, and session ID generation. If a PM-mode checkpoint is found (identified by the `project_manager_mode: true` field), resume with the PM-specific state (active Code Managers, assigned batches, queued WATCH payloads).
...
1. **Read configuration:**
   ...
   governance:
     ...
     parallel_code_managers: 3    # default 3, max concurrent Code Managers
...
After filtering and prioritizing issues, the DevOps Engineer groups them by change type for efficient Code Manager dispatch:
...
### PM Phase 2: Dispatch Code Managers

> **Context Gate — PM Phase 2 Entry:** Execute the Context Gate protocol. Yellow tier: do NOT spawn new Code Managers. Orange/Red: execute Shutdown Protocol.

**Persona:** Project Manager (`governance/personas/agentic/project-manager.md`)

1. **Receive grouped issue batches** from DevOps Engineer RESULT
2. **Determine dispatch count:** min(number of groups, `governance.parallel_code_managers`)
3. **Spawn Code Managers** — one per issue group, each as a background Task agent:
   ...
   Task(
     ...
     isolation: "worktree",
     run_in_background: true,
     prompt: "<Code Manager persona> + <assigned issue group> + <batch scope constraints>"
   )
   ```

   **The Code Manager prompt must include:**
   - Full Code Manager persona instructions
...
### PM Phase 2b: Code Manager Batch Execution

Each Code Manager executes the standard Phase 2-5 pipeline within its batch scope:
...
3. **Phase 4 (Review):** Collect Coder results, run Tester evaluation, security review, PR monitoring
4. **Phase 5 (Merge):** Merge all PRs in its batch

When all issues in the batch are complete, the Code Manager emits a RESULT to the Project Manager with:
...
The Project Manager collects Code Manager results as they arrive:
...
2. **Handle WATCH messages** — if the DevOps Engineer sends a WATCH with new issues:
   - If a Code Manager slot is available and context tier is Green: spawn a new Code Manager
   - Otherwise: queue the WATCH payload
3. **Process queued WATCH payloads** — when an active Code Manager completes, check the queue and spawn a replacement if capacity permits
4. **Detect cross-batch conflicts** — if two Code Managers modify the same files, coordinate by holding the later batch until the earlier one merges
...
1. **Verify all active Code Managers have completed** or handle failures:
   - Completed: process RESULT
   - Failed: log failure, queue batch for next session
   - Timed out: emit CANCEL, wait for partial RESULT, queue remainder
2. **Run session retrospective** — aggregate metrics across all Code Managers
...
All standard constraints apply, plus:
- **Maximum M Code Managers** (where M = `governance.parallel_code_managers`, default 3) — never exceed the configured cap
- **Nested parallelism budget:** Total concurrent agents = M Code Managers x N Coders (per CM). Monitor aggregate context pressure.
- **DevOps Engineer is single-instance** — only one DevOps Engineer runs at a time (background polling mode)
- **Code Managers are batch-scoped** — a Code Manager processes only its assigned issues; it does not scan for additional work
- **WATCH messages are ordered** — process in arrival order; do not skip or reorder
- **PM checkpoint includes CM state** — when writing a checkpoint, include active Code Manager IDs, their assigned batches, and the WATCH queue
